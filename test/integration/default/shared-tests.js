...
test(`[Shared file copying (custom paths)] shared() never uses symlinks by default`, t => {
  t.plan(2)
...
test(`[Shared file copying with plugins (default paths)] shared() copies shared and views, as well as files via plugin`, t => {
  t.plan(
    sharedArtifacts.length +
    getViewsArtifacts.length +
    sharedArtifactsDisabled.length +
...
test(`[Shared file copying (custom paths)] shared() copies static.json but not app.arc (Arc v6+)`, t => {
  t.plan(arcFileArtifacts.length + staticArtifacts.length + 1)
  resetAndCopySharedCustom(t, function () {
    hydrate.shared({}, function (err) {
      if (err) t.fail(err)
...
test(`[Shared file copying (default paths)] shared() copies static.json with @static folder configured`, t => {
...
test(`[Shared file copying (custom paths)] shared() copies static.json with @static folder configured`, t => {
  t.plan(staticArtifacts.length + 3)
  resetAndCopySharedCustom(t, function () {
    // Rewrite app.arc to include @static folder directive
    let arcFile = join(process.cwd(), 'app.arc')
